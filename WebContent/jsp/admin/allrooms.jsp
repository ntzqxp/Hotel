  ...
  <c:choose>
			  ...
			  <c:forEach var="room" items="${sessionData.roomList }" varStatus="status">
					<tr>
					    ...
					    <c:choose>
					   		<c:when test="${room.removed }">
						    <form action="${pageContext.request.contextPath}/controller" method="post" >
					    		<input type="hidden" name=command value="change_room_removed" />
    ...
    <c:if test="${sessionData.currentPage != 1}">
    	<c:url value="/controller" var="URL">
			...
			<c:param name="command" value="to_all_rooms" />
		</c:url> 
	...
	<form action="${pageContext.request.contextPath}/controller"
		method="post">
		<input type="hidden" name="command" value="back_to_admin_rooms" />
